...
class DraggableHome extends StatefulWidget {
  ...
  final double secondHeaderExpandedHeight;

  /// Header Widget: A widget to display Header above body.
  final Widget headerWidget;
  final Widget secondHeaderWidget;
  ...
  final Function? changedAppBarBackIcon;
  ...
  const DraggableHome({
    ...
    this.secondHeaderExpandedHeight = 0.35,
    required this.headerWidget,
    required this.secondHeaderWidget,
...
class _DraggableHomeState extends State<DraggableHome> {
  ...
  ValueNotifier<bool> isExpandHalfScrolled = ValueNotifier(false);
  ...
  Widget build(BuildContext context) {
    ...
    final double secondExpandedHeight = MediaQuery.of(context).size.height * widget.secondHeaderExpandedHeight;
    ...
    return Scaffold(
      ...
      body: ValueListenableBuilder(
        valueListenable: isExpandHalfScrolled,
        builder: (BuildContext context, bool value, Widget? child) {
          return NotificationListener<ScrollNotification>(
            onNotification: (notification) {
              // Check if it's a scroll update notification
              // if (notification is ScrollUpdateNotification) {
              //   // Get the current scroll position and viewport dimensions
              //   double currentScrollPosition = notification.metrics.pixels;
              //   double viewportHeight = notification.metrics.viewportDimension;
              //
              //   // If the scroll position is greater than half the viewport height, trigger an action
              //   if (currentScrollPosition >= viewportHeight / 3.7 && !isExpandHalfScrolled.value) {
              //     isExpandHalfScrolled.value = true;
              //     if (widget.scrollController != null && widget.scrollController!.hasClients) {
              //       widget.scrollController!.animateTo(0, duration: const Duration(milliseconds: 500), curve: Curves.easeInOut);
              //     }
              //   }
              // }
              ...
              if (notification.metrics.axis == Axis.vertical) {
                ...
                //isFullyCollapsed
                if (notification.metrics.extentBefore > (value ? secondExpandedHeight : expandedHeight) - AppBar().preferredSize.height - 40) {
                  if (!(isFullyCollapsed.value)) isFullyCollapsed.add(true);
                  widget.changedAppBarBackIcon?.call(true);
                } else {
                  if ((isFullyCollapsed.value)) isFullyCollapsed.add(false);
                  widget.changedAppBarBackIcon?.call(false);
                }

                isScrolling.value = notification.metrics.pixels <= notification.metrics.minScrollExtent;
              }
              return false;
            },
            child: sliver(context, appBarHeight, fullyExpandedHeight, (value ? secondExpandedHeight : expandedHeight), topPadding),
          );
        },
      ),
          ...
          builder: (BuildContext context, AsyncSnapshot<List<bool>> snapshot) {
            ...
            return SliverAppBar(
              ...
              leading: ValueListenableBuilder(
                ...
                builder: (BuildContext context, bool isScrollingValue, Widget? child) {
                  return isScrollingValue
                      ...
                      background: Container(
                        ...
                        child: fullyExpanded
                            ...
                            : ValueListenableBuilder(
                                valueListenable: isExpandHalfScrolled,
                                builder: (BuildContext context, bool value, Widget? child) {
                                  return value == true ? widget.secondHeaderWidget : widget.headerWidget;
                                },
                              ),
                      ),
                    ),
