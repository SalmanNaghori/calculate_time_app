...
class FontSelectChip extends StatefulWidget {
  ...
  FontSelectChip(this.itemList, {Key? key, required this.selectedItem, required this.onSelectionChanged}) : super(key: key);
    ...
    for (var item in widget.itemList) {
      choices.add(
        GestureDetector(
          ...
          child: Padding(
            padding: EdgeInsets.only(right: Dimensions.w10, bottom: Dimensions.w3, top: Dimensions.w3),
            child: Container(
              height: Dimensions.h42,

              // padding: EdgeInsets.only(left: Dimensions.w12,right: Dimensions.w12),
              decoration: BoxDecoration(
                color: widget.selectedItem == item ? AppColor.primaryColor : Theme.of(context).colorScheme.primaryContainer,
                borderRadius: BorderRadius.all(
                  Radius.circular(Dimensions.r6),
                ),
                border: Border.all(
                  color: widget.selectedItem == item ? AppColor.primaryColor : Theme.of(context).colorScheme.primaryContainer,
                ),
                ...
                children: [
                  ...
                  Text(
                    ...
                    style: context.fontStyleRegular14
                        ?.copyWith(fontFamily: item.fontStyleName, color: widget.selectedItem == item ? AppColor.textWhiteColor : null),
                  ),
                  SizedBox(
